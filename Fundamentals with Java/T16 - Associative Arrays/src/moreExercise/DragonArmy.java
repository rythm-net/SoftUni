    ...
    public static void main(String[] args) {
        ...
        statistic.forEach((key1, value1) -> {
            System.out.printf("%s::(%.2f/%.2f/%.2f)%n", key1,
                    value1.values().stream().map
                            (doubles -> doubles.get(0)).mapToDouble(Double::doubleValue).average().orElseThrow(),
                    value1.values().stream().map
                            (doubles -> doubles.get(1)).mapToDouble(Double::doubleValue).average().orElseThrow(),
                    value1.values().stream().map
                            (doubles -> doubles.get(2)).mapToDouble(Double::doubleValue).average().orElseThrow());
                    value1.forEach((key, value) ->
            System.out.printf("-%s -> damage: %.0f, health: %.0f, armor: %.0f%n", key,
                    value.get(0),
                    value.get(1),
                    value.get(2)));
        });
